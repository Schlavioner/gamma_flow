...
def train_dim_model(data_tr: List[Dict], save_results: bool):
    ...
    loadings_tr = []
    print(
        f"Building loadings from mean spectra of those isotopes: {GlobalVariables.isotopes_tr}"
    )

    dets_train = copy.deepcopy(GlobalVariables.dets_tr)
    for iso in GlobalVariables.isotopes_tr:
        if (
            "simulated+bg" in dets_train
        ):  # use simulated spectra and (measured) pure background spectra
            if iso == "background":  # measured background spectra
                ...
                iso_spectra = [
                    ...
                    for x in data_tr
                    if [iso] == x["labels"]
                    and any(x["detector"] in d for d in dets_train)
                ]
            ...
            iso_spectra = [
                x["spectrum"]
                for x in data_tr
                if [iso] == x["labels"] and x["detector"] in dets_train
            ]
        ...
        if int_mean_iso == 0.0:
            raise ValueError(
                f"ERROR: Cannot normalize mean spectrum of {iso}! Integral: {int_mean_iso}"
            )
        else:
...
def transform_spectra(data: List[Dict]) -> List[Dict]:
    ...
    check_for_nan_and_inf(
        {"loadings": loadings_tr, "spectra": spectra}, "transform_spectra()"
    )
...
def calculate_explained_variance(
    original: Union[List[float], np.ndarray], denoised: Union[List[float], np.ndarray]
) -> float:
    """
    ...
    if original.ndim == 1:  # only one spectrum
        exp_var = explained_variance_score(
            original[GlobalVariables.min_channel_tr :],
            denoised[GlobalVariables.min_channel_tr :],
        )

    if original.ndim == 2:  # multiple spectra
        exp_var = explained_variance_score(
            original[:, GlobalVariables.min_channel_tr :].T,
            denoised[:, GlobalVariables.min_channel_tr :].T,
        )
...
def denoise_spectra(data: List[Dict]) -> Tuple[List[Dict], float]:
    ...
    denoised_spectra = np.dot(
        scores, GlobalVariables.loadings_tr
    )  # transform scores back to denoised spectra

    explained_variance = calculate_explained_variance(
        spectra, denoised_spectra
...
def create_prediction_dict(data_te: List[Dict]) -> List[Dict]:
    ...
    for x in data_te:  # iterate over spectra
        ...
        labels_pred_dict = {
            iso: round(sco, 3)
            for iso, sco in zip(GlobalVariables.isotopes_tr, scores)
            if sco > GlobalVariables.min_scores_norm
        }

        # sort dictionary by scores (descending)
        labels_pred_dict_sort = sorted(
            labels_pred_dict.items(), key=lambda item: item[1], reverse=True
        )

        # change dictionary order (move 'background' to the end)
        labels_pred_dict_sort = [
            item for item in labels_pred_dict_sort if item[0] != "background"
        ]  # all except background
        background_entry = [
            (k, v) for k, v in labels_pred_dict.items() if k == "background"
        ]  # only background
        labels_pred_dict_sort.extend(
            background_entry
        )  # merge together (background at the end of list)
        labels_pred_dict_sort = dict(
            labels_pred_dict_sort
...
def print_multi_label_accuracies(data_te: List[Dict]):
    ...
    for x in data_te:
        ...
        acc_list = [
            sco if iso in x["labels"] else -sco
            for iso, sco in x["labels_pred_dict"].items()
        ]

        # add penalties for missing true labels
        acc_list.extend(
            [
                -1.0 / len(labels_true_i)
                for iso_true in labels_true_i
                if iso_true not in labels_pred_i
            ]
        )

        # sum all elements of acc_list, minimum is 0
        acc = np.max([0.0, np.sum(acc_list)])
    ...
    # b) incomplete, but only-correct predictions: predicted labels < true labels
    n_incomplete = len(
        [x for x, y in zip(labels_te, labels_pred) if set(y) < set(x) and x != y]
    )
    ratio_incomplete = n_incomplete / len(data_te)
    print(
        f"{n_incomplete} incomplete, but only-correct multi-label-classifications ({ratio_incomplete: .1%}). "
    )

    # c) complete, but partially wrong predictions: predicted labels > true labels
    n_wrong = len(
        [x for x, y in zip(labels_te, labels_pred) if set(x) < set(y) and x != y]
    )
    ratio_wrong = n_wrong / len(data_te)
    print(
        f"{n_wrong} complete, but partially wrong multi-label-classifications ({ratio_wrong: .1%}). "
    )

    # d) incomplete and partially wrong predictions: predicted labels include wrong labels and some true labels are missing.
    n_wrong_and_incomplete = len(
        [
            x
            for x, y in zip(labels_te, labels_pred)
            if not set(x) < set(y) and not set(y) < set(x) and x != y
        ]
    )
    ratio_wrong_incomplete = n_wrong_and_incomplete / len(data_te)
    print(
        f"{n_wrong_and_incomplete} incomplete, partially wrong multi-label-classifications ({ratio_wrong_incomplete: .1%}). "
    )
...
def print_single_label_accuracy(data_te: List[Dict]):
    ...
    labels_te_flat = [
        next((x for x in lab if x != "background"), lab[0]) for lab in labels_te
    ]
...
def classify_from_scores(data_te: List[Dict], class_type: str) -> List[Dict]:
    ...
    ):  # keep only the first entry of the dictionary (highest score)
        print_single_label_accuracy(data_te)

    elif (
        class_type == "multi-label"
    ):  # compare all predicted labels with all true labels
        print_multi_label_accuracies(data_te)

    else:
        raise ValueError(
            f"class_type can be single-label or multi-label but you chose {class_type}!"
        )
    return data_te
