...
dir_numpy_raw = join(
    dir_data, "1_numpy_raw"
)  # folder for .np data (before preprocessing)
dir_numpy_ready = join(
    dir_data, "2_numpy_ready"
)  # folder for .np data (after preprocessing)
...
def load_spectral_data(
    dir_input: str | os.PathLike, detectors: List[str], **kwargs
    ...
    exclude_files = kwargs.get(
        "exclude_files", []
    )  # optional parameter: files to be excluded
    include_files = kwargs.get(
        "include_files", []
    )  # optional parameter: files to be included
    if include_files:
    ...
    for name in all_names:  # iterates over files in dir_input
        for det in detectors:
            ...
            data_from_det = [
                x for x in data if x["detector"] == det
            ]  # only spectra of specified detector
            data_new.extend(data_from_det)
    isotopes_det = sorted(
        [sorted(y) for y in set([tuple(x["labels"]) for x in data_new])]
    )
    if len(data_new) == 0:
        raise TypeError(
            f"Could not find data from {dir_input}, {detectors=} with {kwargs=}!"
        )
...
def calc_mean(
    isotope: str, folder: str | os.PathLike, detectors: List[str], norm: bool
):
    """
    ...
    data = np.load(
        join(folder, f"{isotope}.npy"), allow_pickle=True
    )  # imports a list of dicts
    n_channels = len(data[0]["spectrum"])
    if isotope != "background":
        data = [
            s for s in data if s["detector"] in detectors
        ]  # keep only data of specified detectors
    ...
    bg_spectra = np.array(
        [
            s["spectrum"]
            for s in data
            if "background" in s["labels"] and len(s["labels"]) == 1
        ]
    )

    # spectra containing the isotope
    iso_spectra = np.array(
        [s["spectrum"] for s in data if np.any([lab in isotope for lab in s["labels"]])]
    )
    ...
    if len(iso_spectra) > 0:
        ...
        print(
            f"No isotope spectra were found for {isotope}, {detectors}. Returning array of zeros."
        )
    ...
    if len(bg_spectra) > 0:
        ...
        print(
            f"No pure background spectra found for {isotope}, {detectors=}, using background.npy instead."
        )
        pure_bg_data = np.load(
            join(dir_numpy_ready, "background.npy"), allow_pickle=True
        )
...
def remove_empty_or_negative_spectra(data: List[Dict]) -> List[Dict]:
    ...
    spectra[spectra < 0] = 1.0e-6  # replace negative values with (approx.) zero
    ...
    data_new = [
        x for x, inte in zip(data, int_spectra) if inte > 0.0
...
def normalize_spectra(spectra: Union[List, np.ndarray]) -> Union[List, np.ndarray]:
    ...
    if type(spectra) == List:  # check if labels is a nested list
        multiple_spectra = (
            True if any(isinstance(item, list) for item in spectra) else False
        )
    ...
    if multiple_spectra:
        ...
        if np.all(int_spectra > 0):
            spectra_norm = np.divide(
                spectra, np.tile(int_spectra, (spectra.shape[1], 1)).T
            )
            return spectra_norm
        else:
            i = np.where(int_spectra == 0)
            print(
                f"ERROR: Cannot normalize spectra, integral of spectrum {i} is 0! Returning original spectra..."
            )
            return spectra
