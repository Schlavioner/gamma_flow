...
"""(
    ...
    cosine_similarity,
    load_spectral_data,
    remove_empty_or_negative_spectra,
    train_dim_model,
    apply_dim_model,
)"""
...
def enrich_dicts_with_information(list_dict: List[Dict]):
    ...
    for da in list_dict:
        ...
        da["explained_variance"] = explained_variance_score(
            da["spectrum"], da["denoised"]
        )
...
def split_spectra_one_vs_all(
    data: List[Dict], isotopes: List[str], excluded_isotope: str
):
    """
    ...
    if excluded_isotope not in isotopes:
        raise ValueError(
            f"You are trying to remove {excluded_isotope} from the data but \
    ...
    data_one = [
        x for x in data if excluded_isotope in x["labels"]
    ]  # all data of excluded_isotope
    data_other = [
        x for x in data if excluded_isotope not in x["labels"]
    ]  # all data of other isotopes
...
def simulate_outlier(isotope_outlier: str):
    ...
    data_unknown, isotope_unknown, data_known, isotopes_known = (
        split_spectra_one_vs_all(data, all_isotopes, excluded_isotope)
    )
    GlobalVariables.isotopes_tr = (
        isotopes_known  # save only known isotopes to global variable for training
    )
    ...
    for da in data_known:
        combined_list = da["scores_norm_sorted"] + [
            da["scores_norm_mean"],
            da["scores_norm_median"],
            da["explained_variance"],
            da["cosine_similarity"],
            da["scores_norm_abs"],
        ]
...
def check_column_names_match(x_combined, labels_combined, column_names):
    ...
    if x_combined.shape[0] != labels_combined.shape[0]:
        raise ValueError(
            "x_combined and labels_combined must have the same number of rows"
        )
    ...
    if combined.shape[1] != len(column_names):
        raise ValueError(
            f"Mismatch: combined array has {combined.shape[1]} columns, "
            f"but column_names has {len(column_names)} elements. Adjust column_names accordinly manually!"
        )


def fit_logistic_regression_for_outlier_feature(
    df_combined: pd.DataFrame, feature: str
):
    """
    ...
    k_guess = -1.0  # guess slope (this can be adjusted if needed)
