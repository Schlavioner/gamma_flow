...
def saveplot(save_plot: bool, name: str, title: str):
    ...
    if save_plot:
        try:
            if (
                "dir_plots" not in globals()
            ):  # Check if dir_plots is defined in the global scope
                raise NameError(
                    "The directory for saving plots ('dir_plots') is not defined."
                )
            ...
            isotope = (
                name.rsplit("_", 1)[0]
                if any(
                    name.endswith(f"_{det}") for det in GlobalVariables.all_detectors
                )
                else name
            )
...
def get_color_list(labels: List[str]) -> Tuple[List, Dict[str, float]]:
    ...
    nested = (
        True if any(isinstance(item, list) for item in labels) else False
    )  # check if labels is a nested list
    if nested:
        distinct_labels = sorted(
            set(tuple(label) for label in labels)
    ...
    distinct_colors = [
        plt.get_cmap("rainbow")(1.0 * i / denomi) for i in range(n_iso)
    ]  # pick n_iso distinct colors
...
def plot_mean_spectra_by_isotope_and_detector(
    iso_list: List[str], zoom_in: bool, save_plots: bool
):
    """
    ...
    for iso in iso_list:  # iterates over isotopes
        ...
        for det in (
            GlobalVariables.all_detectors
        ):  # iterates over all detectors (e.g. ['left', 'right', 'simulated'])
            # mean and standard deviation of all spectra in <name>.npy, isotope and background spectra seperately
            mean_bg, std_bg, mean_iso, std_iso = calc_mean(
                iso, dir_numpy_ready, [det], norm=False
            )
            ...
            if int_iso != 0.0:
                ...
                if (
                    "background" in iso or det == "simulated"
                ...
                axes[1].set_title(
                    f"Mean spectra of {iso} (background subtracted, normalized)"
                )
                axes[1].set_xlabel("energy / keV")
                axes[1].set_xlim(0, np.max(energies))
                if zoom_in:
                    axes[1].set_xlim(
                        0, np.max(energies) / 10
                    )  # option to zoom into lower channels
        ...
        if (
            len(means_dict.keys()) > 1
        ):  # multiple means per isotope / multiple detectors
            ...
            for idx, (i, j) in enumerate(combinations):
                ...
                axes[1].annotate(
                    f"Cos similarity betw. {dettxt} : {cos_sim: .2f}",
                    xy=(0.02, 0.91 - idx * 0.1),
...
def plot_example_spectra_by_isotopes(iso_list: List[str], n_spectra, save_plot: bool):
    ...
    _, color_dict = get_color_list(
        iso_list
    )  # dictionary to assign colors to different isotopes in plots

    for iso in iso_list:
        data, _ = load_spectral_data(
            dir_numpy_ready, GlobalVariables.all_detectors, include_files=f"{iso}.npy"
        )
        spectra = [x["spectrum"] for x in data]
        labels = [x["labels"] for x in data]
        if n_spectra != "all":
            indices = np.random.choice(
                np.arange(len(labels)), n_spectra
            )  # pick n_spectra spectra randomly
            spectra = [spectra[i] for i in indices]
            labels = [labels[i] for i in indices]
        iso_spectra = np.array(
            [spec for spec, lab in zip(spectra, labels) if iso in lab]
        )
        ...
        if len(iso_spectra) > 0:
            ...
            print(
                f"No spectra labelled as {iso} found in {iso}.npy, cannot be plotted."
            )


def plot_cos_sim_matrix_means(
    cosine_similarity_matrix: np.ndarray, names: List[str], threshold: float
):
    """
    ...
    if not -1.0 <= threshold <= 1.0:
        raise ValueError(
            f"The threshold was set to {threshold} but has to be a number between -1 and 1!"
        )
    ...
    if (
        cosine_similarity_matrix.shape[0] != cosine_similarity_matrix.shape[1]
        or cosine_similarity_matrix.shape[0] != n_names
    ):
        raise ValueError(
            "The cosine similarity matrix must be square and match the length of the names list."
        )
    ...
    for i in range(n_names):
        for j in range(n_names):
            ...
            if i >= j:  # only lower left triangle
                ...
                ax.text(
                    j, i, text, va="center", ha="center", color=color, fontsize=10
                )  # write value of cosine similarity into each square

                # Draw orange box around values of same isotope, different detector that are too low (not similar enough)
                if (
                    names[i].split("_")[0] == names[j].split("_")[0]
                    and names[i] != names[j]
                ):
                    if cos_sim < threshold:
                        print(
                            f"Not similar enough: {names[i]} and {names[j]}, cosine similarity={cos_sim:.3} in row {i}, column {j}"
                        )
                        ax.add_patch(
                            plt.Rectangle(
                                (j - 0.5, i - 0.5),
                ...
                if names[i].split("_")[0] != names[j].split("_")[0]:
                    if cos_sim >= threshold:
                        print(
                            f"Too similar: {names[i]} and {names[j]}, cosine similarity={cos_sim:.3}, in row {i}, column {j}"
                        )
                        ax.add_patch(
                            plt.Rectangle(
                                (j - 0.5, i - 0.5),
...
def plot_loadings_subplots(save_plot: bool):
    ...
    for k, iso in enumerate(isotopes):
        ...
        axes[k].plot(
            loadings_norm[k, :], c="grey", alpha=0.8, label=f"PC {k + 1}: {iso}"
        )

        # plot normalized loadings (only channels > min_channel_tr)
        axes[k].plot(
            x[min_channel:],
            loadings_norm[k, min_channel:],
            linewidth=3,
            c=color_list[k],
            alpha=0.8,
            label=f"channels ≥ {min_channel}",
        )
    ...
    axes[k].set_xlabel(
        "Energy channel", fontsize=15
    )  # label for x axis: lowest subplot only
...
def plot_confusion_matrix(data_te: List[Dict], class_type: str, save_plot: bool):
    ...
    if class_type not in ["single-label", "multi-label"]:
        raise ValueError(
            f'Invalid class_type: {class_type}. Must be either "single-label" or "multi-label".'
        )

    if class_type == "single-label":
        labels_true = [
            x["labels"][0] for x in data_te
        ]  # keep only first label (either isotope or pure background)
        labels_predict = [
            list(x["labels_pred_dict"].keys())[0] for x in data_te
        ]  # keep only first key of dictionary
    elif class_type == "multi-label":
        labels_true = [
            " ".join(sorted(x["labels"])) for x in data_te
        ]  # converts ['Am241', 'background'] to 'Am241 background'
        labels_predict = [
            " ".join(sorted(list(x["labels_pred_dict"].keys()))) for x in data_te
        ]
    ...
    confusion_matrix = np.zeros(
        (len(class_labels_predict), len(class_labels_true)), dtype=int
    )
    ...
    ax = sns.heatmap(
        ...
        fmt="d",
        cmap="YlGnBu",
        cbar_kws={"label": "counts", "shrink": 0.5},
        xticklabels=class_labels_true,
        yticklabels=class_labels_predict,
    )
...
def plot_misclassified_spectra(data_te: List[Dict], class_type: str, save_plot: bool):
    ...
    for i, da in enumerate(data_err):
        ...
        scores_dict = {
            iso: score for iso, score in zip(GlobalVariables.isotopes_tr, scores)
        }

        if class_type == "single-label":
            label_true = da["labels"][0]  # extract first true label
            label_pred = list(da["labels_pred_dict"].keys())[
                0
            ]  # extract dominant predicted label
        ...
        ax_left.plot(
            spectrum, c="black", label=labeltext_left
        )  # plot misclassified spectrum
        ...
        loadings_dict = {
            iso: pc
            for iso, pc in zip(GlobalVariables.isotopes_tr, GlobalVariables.loadings_tr)
        }
        ...
        if class_type == "single-label":
            ...
            ax_left.plot(
                pc_predict, c="firebrick", alpha=0.6, label=f"predicted: {label_pred}"
            )
            ax_left.plot(
                pc_true, c="green", alpha=0.6, label=f"correct label: {label_true}"
            )

            # annotate predicted and true scores in left subplot
            annot_text_left0 = (
                f"scores predict ({label_pred}): {scores_dict[label_pred]: .3} \n"
            )
            annot_text_left1 = (
                f"scores true ({label_true}) : {scores_dict[label_true]: .3}"
            )
            ...
            for iso in da["labels_pred_dict"].keys():
                ...
                pc_predict = normalize_spectra(pc_predict)
                ax_left.plot(
                    pc_predict, c="firebrick", alpha=0.6, label=f"predicted: {iso}"
                )
            ...
            for iso in da["labels"]:
                if iso in loadings_dict.keys():
                    ...
                    pc_true = normalize_spectra(pc_true)
                    ax_left.plot(
                        pc_true, c="green", alpha=0.6, label=f"correct label: {iso}"
                    )
                else:
                    print(
                        f"ERROR: No principal component found for {iso}. Maybe {iso} was not included in model training?"
                    )
            ...
            annot_text_right0 = {
                iso: round(sco, 2) for iso, sco in da["labels_pred_dict"].items()
            }
            ...
            annot_text_right = (
                f"scores predict: {wrap_text(str(annot_text_right0), wrap_width)}\n"
        ...
        ax_right.annotate(
            f"cosine similarity: {cos_sim: .3}", xy=(0.1, 0.9), xycoords="axes fraction"
        )
        ax_right.legend()

    plt.tight_layout()
    saveplot(
        save_plot, "_".join(GlobalVariables.isotopes_tr), title="Misclassified_spectra"
    )
...
def plot_misclassification_statistics(data_te: List[Dict], class_type: str):
    ...
    for i, data in enumerate(
        [data_corr, data_err]
    ):  # iterate over correct/incorrect data
        if not data:
        ...
        cos_sim_list = [
            cosine_similarity(a, b) for a, b in zip(spectra, denoised_spectra)
        ]
    ...
    xlabels = [
        "explained variance ratio between denoised & original spectrum)",
    ...
    for i, (q_corr, q_err) in enumerate(zip(quantities_corr, quantities_err)):
        ...
        n_corr, bins_corr, _ = axes2.hist(
            q_corr, bins=bins
        )  # histogram data for correct predictions
        n_err, bins_err, _ = axes2.hist(
            q_err, bins=bins
        )  # histogram data for incorrect predictions
        ...
        axes[i].axvline(
            threshs[i], color="black", linewidth=2.0
...
def plot_scores_scatter_matrix(
    data_tr: List[Dict],
    data_te: List[dict],
    class_type: List[dict],
    n_dim_max: int,
    only_errs: bool,
    save_plot: bool,
):
    ...
    ind_tr = list(
        np.random.choice(np.arange(n_spectra_tr), min(n_spectra_tr, 5000))
    )  # limit displayed training data
    ind_te = list(
        np.random.choice(np.arange(n_spectra_te), min(n_spectra_te, 300))
    )  # limit displayed test data
    ...
    n_rows = n_cols = min(
        n_dim_real, n_dim_max
    )  # limit number of displayed principal components
    fig, axes = plt.subplots(
        n_rows - 1, n_cols - 1, figsize=(2 * (n_rows - 1), 2 * (n_cols - 1))
    )

    for j in range(1, n_rows):  # iterates over rows (dimensions in latent space)
        for k in range(
            n_cols - 1
        ):  # iterates over columns (dimensions in latent space)
            if j <= k:  # row <= column
                axes[j - 1][k].axis(
                    "off"
                )  # show only lower left triangle of the figure

            else:  # row > column (lower left triangle): oppose dimension j and dimension k
                axes[j - 1][k].scatter(
                    scores_tr[:, k], scores_tr[:, j], s=5, color=c_tr, label=labels_tr
                )  # plot training data
    ...
    fig.legend(
        handles=patches + [tr_marker, te_marker], bbox_to_anchor=(1, 0.8), fontsize=15
    )
    ...
    concatname = "_".join(
        GlobalVariables.isotopes_tr
    )  # concatenate isotopes to one string as filename
...
def plot_mean_scores_barplot(data_te: List[Dict], class_type: str, save_plot: bool):
    ...
    if class_type == "multi-label":
        ...
        n_iso_tr = len(
            GlobalVariables.isotopes_tr
        )  # number of isotopes used in model training
        x = np.arange(n_iso_tr)  # set up x axis
        width = 1 / (n_iso_tr + 1)  # calculate width for each x tick

        for ll, iso in enumerate(
            GlobalVariables.isotopes_tr
        ):  # iterates over isotopes used in model training
            # find all scores labelled as iso or iso + background
            ind_iso = [
                ii
                for ii, lab in enumerate(labels_te)
                if iso in lab and set(lab).issubset({iso, "background"})
            ]
            ...
            if len(scores_iso) > 0:  # test if any scores are found for isotope iso
                ...
                print(
                    f"ratio of mean {iso} scores on {iso} axis: {mean_score_iso[ll]:.2}"
                )  # print score of the correct axis

                # normalize mean scores and standard deviation
                int_mean_score = np.sum(np.abs(mean_score_iso))
                mean_score_iso = (
                    mean_score_iso / int_mean_score
                )  # normalize mean scores
                std_score_iso = (
                    np.std(scores_iso, axis=0) / int_mean_score
                ...
                plt.xlabel(
                    "axis in latent space, corresponding to isotope", fontsize=15
                )
                plt.ylabel("share of scores", fontsize=15)
                plt.title(
                    f"Share of scores (mean value, aggregated by true labels)",
                    fontsize=20,
                )
...
def identify_misclassifications(
    data_te: List[Dict], class_type: str
    ...
    if class_type == "single-label":
        ...
        if np.any(
            [len(lab) > 2 for lab in labels_te]
        ):  # check if data_te contains multi-label data
            print(
                f"You chose {class_type=} but your test data contain multi-label spectra!"
            )

        # filter out background from isotope spectra (convert ['Am241', 'background'] to ['Am241'])
        labels_te_without_bg = [
            l if l == ["background"] else list(filter(lambda x: x != "background", l))
            for l in labels_te
        ]
        ...
        data_corr = [
            x for x in data if list(x["labels_pred_dict"].keys())[0] == x["labels"][0]
        ]
        data_err = [
            x for x in data if list(x["labels_pred_dict"].keys())[0] != x["labels"][0]
        ]
        ...
        data_corr = [
            x for x, true, pred in zip(data, labels_te, labels_pred) if pred == true
        ]
        data_err = [
            x for x, true, pred in zip(data, labels_te, labels_pred) if pred != true
        ]
