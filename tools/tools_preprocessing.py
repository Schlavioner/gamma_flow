...
from tools.plotting import plot_original_and_rebinned_example_spectrum
...
def check_folder_structure():
    ...
    """
    essential_folders = [dir_plots, dir_trained_models, dir_data, dir_numpy_raw, dir_numpy_ready]
    ...
    if not os.path.isfile(isolist_path):
        ...
        if 'You need to manually list the isotopes' in content:
            ...
            print(f'Seems like you have successfully edited the txt-file {isolist_path}. Well done! \n')  
...
def clear_preprocessed_data(paths):
    ...
    for folder_path in paths:
        ...
        for filename in os.listdir(folder_path):  # iterates through folder content
            ...
            try:
                if os.path.isfile(file_path) or os.path.islink(file_path):  # element is a file
                    os.remove(file_path)
...
def read_isotope_list() -> List[str]:
    ...
    list_of_isotopes = [x.replace("\n", "") for x in list_of_isotopes]  # remove line breaks
    list_of_isotopes = sorted(list(dict.fromkeys(list_of_isotopes)))  # remove duplicates, sort alphabetically
    
    return list_of_isotopes


def rebinning(all_files_list:List[str], std_calib:Dict, show_plot:bool) -> Dict[str, List[Dict]]:
    """
    ...
    for filename in all_files_list:  # iterates over isotopes   
        ...
        std_energies = std_calib['offset'] + std_calib['slope'] * i + std_calib['quad'] * i**2   
        
        for jj, (dicty_ori, dicty_rebinned) in enumerate(zip(data_ori, data_rebinned)):  # iterates over spectra
            ...
            ori_energy = ori_calib['offset'] + ori_calib['slope'] * i + ori_calib['quad'] * i**2  # original energ
            
            # define interpolation function 
            interp_func = interp1d(ori_energy, spectra[jj, :], kind='linear', bounds_error=False, fill_value=0, axis=0)
            rebinned_spectrum = interp_func(std_energies) 
            rebinned_spectrum[rebinned_spectrum < 0] = 1e-6  # replace negative values

            dicty_rebinned['spectrum'] = rebinned_spectrum  # update spectrum in dictionary
            dicty_rebinned['calibration'] = std_calib  # update energy calibration in dictionary
        
        data_rebinned = [x for x in data_rebinned if not np.any(np.isnan(x['spectrum']))]  # check for NaNs
        dict_of_data[filename] = data_rebinned  # list of dicts with filename and data (data is a list of dicts)
        
        if show_plot:  # plot original and rebinned spectrum
            plot_original_and_rebinned_example_spectrum(filename, data_ori, data_rebinned)
    return dict_of_data


def concatenate_isotope_datasets(dict_of_data:Dict[str, List[Dict]]) -> Dict[str, List[Dict]]:
    """
    ...
    bg_synonyms = ['background', 'Background', 'Untergrund', 'untergrund', 'Hintergrund', 'hintergrund']
    for isotope in GlobalVariables.all_isotopes:  # iterate over isotopes (keywords by which datasets are concatenated)
        for det in GlobalVariables.all_detectors:  # iterate over detectors
            ...
            if isotope == 'background':
                iso_files = [key for key in filename_list if any(syn in key for syn in bg_synonyms) and det in key]
            else: 
                iso_files = [key for key in filename_list if isotope in key and det in key]
            ...
            if len(iso_files) > 1:  # more than one file per isotope and detector
                ...
                data_extracted = dict_of_data[iso_files[0]]  # extract data from dict_of_data
                dict_of_data_concat1[new_name] = data_extracted  #  write to dict_of_data_concat1
    return dict_of_data_concat1
    

def concatenate_detector_datasets(dict_of_data:Dict[str, List[Dict]]) -> Dict[str, List[Dict]]:
    """
    ...
    filename_list = sorted(dict_of_data.keys())  # defines which files exist in dict_of_data
...
def calc_cos_sim_matrix_means(iso_list: List[str]) -> Tuple[np.ndarray, List[str]]:
    ...
    for iso in iso_list: 
        ...
        det_iso = sorted(set([d['detector'] for d in data_iso]))  # unique list of detectors
        for det in det_iso: 
            name = f'{iso}_{det}'
            _, _, mean_iso, _ = calc_mean(iso, dir_numpy_ready, [det], norm=False)
            all_means_dict[name] = mean_iso
        if len(det_iso) == 0:
            print(f'No spectra labelled as {iso} found in {iso}.npy, cannot be included in the cosine similarity matrix! \n')
